...
int POS_Raster_DownloadAndPrintBmp(int hPort,int nPortType,
	char *pszPath,    // null-terminated string	
	int nOrgx        // x-coordinate of starting position
	);

int POS_Raster_DownloadAndPrintBmp_A(int hPort,int nPortType,
	unsigned char *image_buf,    // pointer to the buffer of the bmp	
	int nOrgx        		     // x-coordinate of starting position
	);
...
int POS_SetTimeOuts(
	int hPort,
	int nPortType,
	int nReadTimeOuts,	//Timeouts of read for the USB port
	int nWriteTimeOuts	//Timeouts of write for the USB port
	);
